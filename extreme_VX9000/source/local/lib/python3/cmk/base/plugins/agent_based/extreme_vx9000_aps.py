#!/usr/bin/env python3
# -*- encoding: utf-8; py-indent-offset: 4 -*-

# (c) Andreas Doehler <andreas.doehler@bechtle.com/andreas.doehler@gmail.com>
# License: GNU General Public License v2

from typing import Dict, NamedTuple, Mapping, List, Optional

from cmk.utils import debug

from .agent_based_api.v1 import (
    register,
    Result,
    Service,
    SNMPTree,
    State,
    TableRow,
    startswith,
    any_of,
)
from .agent_based_api.v1.type_defs import (
    CheckResult,
    DiscoveryResult,
    InventoryResult,
    StringTable,
)


class WLCAp(NamedTuple):
    #status: str
    #home_state: str
    ip_addr: str
    #registration: str
    zone: str
    hw_ver: str
    #sw_ver: str
    serial: str
    location: str
    clients: str
#    uptime: str


Section = Dict[str, WLCAp]



def parse_extreme_vx9000_aps(string_table: List[StringTable]) -> Section:
    client_dict = {}
    for element in string_table[1]:
        wlanap = str(element[0])
        if wlanap in client_dict.keys():
            client_dict[wlanap] += 1
        else:
            client_dict[wlanap] = 1
    wlc_dict = {
        ap_name: WLCAp(
            #status=ap_status,
            #status=client_dict.get(ap_name, 0),
#            uptime=client_dict.get(ap_name, 1),
            #registration=ap_unprovisioned,
            ip_addr=ap_ip,
            zone=ap_zone,
            hw_ver=ap_hwversion,
            #serial=ap_serial,
            location=ap_sysloc,
            #sw_ver=ap_swversion,
            #home_state=ap_homestate,
            clients=ap_clients,
            #clients=client_dict.get(ap_name, 0),
            serial=client_dict.get(ap_name, 0),
        )
        for (
            ap_name,
            # ap_status,
            # ap_unprovisioned,
            ap_ip,
            ap_zone,
            ap_hwversion,
            ap_clients,
            # ap_serial,
            ap_sysloc,
            # ap_homestate,
            # ap_swversion,
        ) in string_table[0]
    }
    if debug.enabled():
        print(string_table)
        
    return wlc_dict


DETECT_EXTREM_WLC = any_of(
    startswith(".1.3.6.1.2.1.1.2.0", ".1.3.6.1.4.1.388.50"),
    startswith(".1.3.6.1.2.1.1.2.0", ".1.3.6.1.4.1.1916.2.294"),
)
#detect=startswith(''.1.3.6.1.2.1.1.1.0', 'VX9000'),  # iso.3.6.1.2.1.1.1.0 = STRING: "VX9000 Wireless Controller, Version 7.7.1.5-003R MIB=01a"


register.snmp_section(
    name="extreme_vx9000_aps",
    parse_function=parse_extreme_vx9000_aps,
    detect=DETECT_EXTREM_WLC,
    fetch=[
        SNMPTree(
            base=".1.3.6.1.4.1.388.50.1.4.2.25.1.1.1",
            oids=[
                "9",  # Name
                #"22",  # Status                 ????
                #"23",  # Unprovisioned          ???
                "13",  # IpAddress
                "10",  # Zone
                "6",  # HWVersion = Type
                "4",  # wingStatsDevWlApInfoNumClients SerialNumber             ???
                "11",  # Location
                #"20",  # HomeState     ???
                #"7",  # SWVersion      ????
            ],
        ),
        SNMPTree(
            base=".1.3.6.1.4.1.388.50.1.4.2.4.1.1",
            oids=[
                #"12", # AP Client is connected to
                #'7',  # wingStatsDevOnline
                '2',  # wingStatsDevSerialNo
#                '14',   # wingStatsDevSysUptime
            ],
        ),
    ],
)

# AP Name       .1.3.6.1.4.1.4329.15.3.5.1.2.1.2
# AP Serial     .1.3.6.1.4.1.4329.15.3.5.1.2.1.4
# AP HomeState  .1.3.6.1.4.1.4329.15.3.5.1.2.1.20   local(1),foreign(2)
# AP State      .1.3.6.1.4.1.4329.15.3.5.1.2.1.22   active(1),inactive(2)
# AP Reg State  .1.3.6.1.4.1.4329.15.3.5.1.2.1.22   approved(1),pending(2)
# AP SW Version .1.3.6.1.4.1.4329.15.3.5.1.2.1.7
# AP AddAssign  .1.3.6.1.4.1.4329.15.3.5.1.2.1.12   dhcp(1),static(2)
# AP IP         .1.3.6.1.4.1.4329.15.3.5.1.2.1.14
# AP HW Version .1.3.6.1.4.1.4329.15.3.5.1.2.1.17
# AP Location   .1.3.6.1.4.1.4329.15.3.5.1.2.1.30
# AP Zone       .1.3.6.1.4.1.4329.15.3.5.1.2.1.33
# AP SecureTun  .1.3.6.1.4.1.4329.15.3.5.1.2.1.41
# AP SSH enab   .1.3.6.1.4.1.4329.15.3.5.1.2.1.43


def discover_extreme_vx9000_aps(section: Section) -> DiscoveryResult:
    for ap_name, _ap_data in section.items():
        yield Service(item=ap_name)


def check_extreme_vx9000_aps(item: str, section: Optional[Section]) -> CheckResult:
    if not section:
        return

    ap_data = section.get(item)
    if not ap_data:
        return

    map_state = {
        "1": (0, "up"),
        "2": (2, "down"),
    }
    map_home = {
        "1": "local",
        "2": "foreign",
    }

    state, state_readable = map_state[ap_data.status]
    infotext = f"Status: {state_readable}"
    if ap_data.clients:
        infotext += f", Clients: {ap_data.clients}"
    if ap_data.zone:
        infotext += f", Zone: {ap_data.zone}"
    if ap_data.location:
        infotext += f", System location: {ap_data.location}"
    #if ap_data.home_state:
    #    infotext += f", Home state: {map_home[ap_data.home_state]}"
    yield Result(
        state=State(state),
        summary=infotext,
    )

    #if ap_data.registration == "2":
    #    yield Result(
    #        state=State.WARN,
    #        summary="Registration pending: yes",
    #    )


def cluster_check_extreme_vx9000_aps(
    item: str, section: Mapping[str, Optional[Section]]
) -> CheckResult:
    found = []
    for node, node_section in section.items():
        results = list(check_extreme_vx9000_aps(item, node_section))
        if results:
            found.append((node, results[0]))

    if not found:
        yield Result(state=State(3), summary="AP not found on any controller")
        return

    best_state = State.best(*(result.state for _node, result in found))
    best_running_on, best_result = [(n, r) for n, r in found if r.state == best_state][
        -1
    ]

    yield best_result
    if best_running_on and best_state != State.CRIT:
        yield Result(state=best_state, summary=f"associated on: {best_running_on}")


register.check_plugin(
    name="extreme_vx9000_aps",
    service_name="AP %s",
    discovery_function=discover_extreme_vx9000_aps,
    check_function=check_extreme_vx9000_aps,
    cluster_check_function=cluster_check_extreme_vx9000_aps,
)


def inventory_extreme_vx9000_aps(section: Section) -> InventoryResult:
    path = ["networking", "wlan", "controller", "accesspoints"]
    for ap_name, ap_data in section.items():
        yield TableRow(
            path=path,
            key_columns={"name": ap_name},
            inventory_columns={
                "ip_addr": ap_data.ip_addr,
                "group": ap_data.zone,
                "model": ap_data.hw_ver,
                "serial": ap_data.serial,
                "sys_location": ap_data.location,
            },
        )


register.inventory_plugin(
    name="extreme_vx9000_aps",
    inventory_function=inventory_extreme_vx9000_aps,
)
